...
//$eazyAdUnblockerObj = new EazyAdUnblocker(); //oops techniques
...
class EazyAdUnblocker
{
	...
	public function eazy_ad_unblocker_dialog_head_func()
	{
		...
		wp_enqueue_style("eazy-custom-ui-css", plugins_url('css/style.css.php', __FILE__), array(), '1.1.5');
	}
	...
	public function eazy_ad_unblocker_add_jquery_ui()
	{
		...
		wp_enqueue_script( "eazy_custom", plugins_url("js/custom.js", __FILE__), array('jquery'), '1.1.5', true );
	}
	...
	public function eazy_ad_unblocker_get_content($encoded_html)
	{
		...
		//YARPP Problem Dec 2020
		
		$content = $content."<!--noyarpp-->";
		
		//End YARPP problem Dec 2020
	...
	public function eazy_ad_unblock_admin_manage()
	{
		...
		if(!empty($_POST["unblocker_save"]))
		{
			...
			if(empty($validate_msg))
			{
				...
				"unblocker_width"=>$eff_width, "eazy_style_name"=>$popup_style_preview);
	...
	public function eazy_ad_unblocker_hide_restore_folder_callback()
	{
		...
		if(!empty($_POST['hide_eazy_ad_unblocker']))
		{
			...
			else
			{
				...
				if(isset($_SESSION["eazy_ad_rand_words"]))
				{
					$dest_folder1 = $_SESSION["eazy_ad_rand_words"][6].'-'.$dest_folder;
				}
				else{
					$dest_folder1 = $dest_folder;
				}
				
				//check for regex
				//if(!ctype_alnum($dest_folder))
				$matches = array();
				if(!preg_match_all('/^[\p{L}\p{N}_]+$/u', $dest_folder, $matches, PREG_PATTERN_ORDER)) //July 23, 2021
				{
					$validate_msgs[] = __("Sorry! Folder name can only have letters, numbers and underscores!", "eazy-ad-unblocker");
				}
				else
				{
					...
					if($this->eazy_ad_unblocker_check_folder_available($eazy_ad_unblocker_plugins_folder, $dest_folder)
					&& $this->eazy_ad_unblocker_check_folder_available($eazy_ad_unblocker_plugins_folder, $dest_folder1)
					)
					{
						$eazy_ad_unblocker_hide_restore = $this->eazy_ad_unblocker_hide_restore_folder($dest_folder1);
